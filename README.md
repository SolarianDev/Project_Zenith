...
## ⚙️ How It Works (The User Perspective)
...
### Storage Redirection
Restricted accounts usually have strict quotas on `$HOME`. The script explicitly generates `storage.conf` to redirect the heavy lifting:
```ini
...
In a standard Docker/Podman setup, the container uses **OverlayFS**, a filesystem that efficiently layers files. However, OverlayFS historically required root privileges to mount.

Because Project Zenith operates in a fully unprivileged environment (often without `/etc/subuid` or `/etc/subgid` delegations that Podman usually relies on), the script forces the **`vfs` (Virtual File System)** storage driver.
...
While Distrobox does an excellent job passing through USB controllers (as seen in the `--device /dev/input:/dev/input` flag for controller support), you cannot easily run `systemd` inside these rootless containers. Background services (like `sshd`, `nginx`, or `postgresql` daemons) must usually be started manually in the foreground rather than relying on `systemctl`.